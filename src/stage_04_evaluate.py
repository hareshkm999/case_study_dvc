...
def evaluate(config_path, params_path):
    ...
    test_y = test_data["90_days_repeat"]
    test_x = test_data.drop("90_days_repeat", axis=1)
    ...
    predicted_values = grid.predict(test_x)
    # Capturing recall
    f1 = f1_score(test_y, predicted_values)
    print("fi score is {}".format(f1))
    recall = recall_score(test_y, predicted_values)
    print("recall score is {}".format(recall))
    precison = precision_score(test_y, predicted_values)
    print("precison score is {}".format(precison))
